...
<Figure
  src="/articles/f1-bayesian-driver-rankings/posterior-predictive.png"
  alt="Density plot overlaying the actual distribution of finish residuals (blue) with the model's simulated residuals drawn from its learned parameters (orange). The two curves overlap closely across the full range, with tails beyond ±20 truncated for display."
  caption="Posterior predictive check. Observed residuals in blue, model-simulated residuals in orange. Close overlap confirms the StudentT likelihood captures the shape of real F1 race outcomes."
  aspect="1024 / 614"
/>
